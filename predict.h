...
PredictOption::PredictOption(int argc, char **argv) {
	...
	if (argc == 5) {
		dst_path = new char[strlen(argv[4]) + 1];
...
template<int nr_id>
void predict(Model<nr_id> *model, char *test_path, char *dst_path) {
	Matrix<nr_id> Te(test_path);
...
void predict(int argc, char **argv) {
	PredictOption option(argc, argv);
	Model<nr_id> model(option.model_path);
	predict<nr_id>(&model, option.test_path, option.dst_path);
}